...
import { state, addCoins, setContestProgress, addParrotTrophy, addParrot } from './gameState.js';
...
export function renderContestsTab() {
    ...
    const selectedParrotId = state.selectedParrotId;
    const parrots = state.parrots;
    ...
    const contestProgress = state.contestProgress;
...
export async function enterContest(tierIndex, saveGameFn, updateStatsFn, checkAchievementsFn) {
    ...
    setContestProgress(selectedParrotId, tierIndex, { placed: placement, coins: coinsWon, badge });
    ...
    // Add trophy
    if (badge) {
        addParrotTrophy(selectedParrotId, { tier: tierIndex, badge, placement });
    }
