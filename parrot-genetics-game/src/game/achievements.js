// ChromaWing - Achievements Module
// v2.0
...
export const ACHIEVEMENTS = {
    ...
    'contest_beginner': {
        ...
        category: 'contests',
        check: () => {
            return Object.values(state.contestProgress).some(pp => pp[0] && pp[0].placed <= 3);
        }
    },
    'contest_all_tiers': {
        ...
        category: 'contests',
        check: () => {
            return Object.values(state.contestProgress).some(pp => {
    ...
    'elite_champion': {
        ...
        isWinCondition: true,
        check: () => {
            return Object.values(state.contestProgress).some(pp => pp[4] && pp[4].placed === 1);
        }
    },
    ...
    'beautiful_parrot': {
        ...
        category: 'breeding',
        check: () => {
            return state.parrots.some(p => p.calculateBeauty().score >= 100);
        }
    },
    'stunning_parrot': {
        id: 'stunning_parrot',
        ...
        category: 'breeding',
        check: () => {
            return state.parrots.some(p => p.calculateBeauty().score >= 150);
        }
    },
    'perfect_parrot': {
        id: 'perfect_parrot',
        ...
        isWinCondition: true,
        check: () => {
            return state.parrots.some(p => p.calculateBeauty().score >= 200);
        }
    },
    'gradient_master': {
        id: 'gradient_master',
    ...
    'full_rarity_set': {
        ...
        category: 'collection',
        check: () => {
            const rarities = new Set(state.parrots.map(p => p.calculateRarity()));
    ...
    'generation_5': {
        ...
        category: 'breeding',
        check: () => state.parrots.some(p => p.generation >= 5)
    },
    'generation_10': {
        id: 'generation_10',
        ...
        category: 'breeding',
        check: () => state.parrots.some(p => p.generation >= 10)
    },
    ...
    'all_achievements': {
        ...
        check: () => {
            ...
            return allIds.every(id => state.achievements.unlocked.includes(id));
        }
    }
};
...
export function checkAchievements() {
    ...
    for (const [id, achievement] of Object.entries(ACHIEVEMENTS)) {
        ...
        if (state.achievements.unlocked.includes(id)) continue;
...
export function showWinConditionModal(achievement) {
    setTimeout(() => {
        ...
        const completedWinConditions = allWinConditions.filter(a => state.achievements.unlocked.includes(a.id));
        ...
        allWinConditions.forEach(wc => {
            const completed = state.achievements.unlocked.includes(wc.id);
...
export function getAchievementStats() {
    ...
    const completedWinConditions = winConditions.filter(a => state.achievements.unlocked.includes(a.id));
